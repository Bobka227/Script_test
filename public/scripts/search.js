...
'A healthy and delicious salad with grilled chicken, fresh vegetables, and a light lemon dressing.',
	//             category: "lunch"
	//         },
	//         {
	...
	document.addEventListener('DOMContentLoaded', async function () {
		...
		// Получаем рецепты с сервера через recipes.php
		async function fetchRecipes() {
			try {
				...
				recipesAll = await response.json()
			} catch (error) {
				console.error('Ошибка:', error)
			}
		}

		// Загружаем рецепты с сервера при инициализации
		await fetchRecipes()
		...
		filterButtons.forEach(button => {
			button.addEventListener('click', function () {
				...
				if (activeFilter === category) {
					...
					recipesSection.classList.add('d-active')
					recipesSection.classList.remove('d-none')
					const filteredRecipes = recipesAll.filter(
